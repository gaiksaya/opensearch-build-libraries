...
void call(Map args = [:]) {
    ...
    withCredentials([string(credentialsId: "${args.apiKey}", variable: 'API_KEY')]) {
        sh """gem cert --add `ls *pem` && cd ${releaseArtifactsDir} && gem install `ls *.gem` -P HighSecurity && curl --fail --data-binary @`ls *.gem` -H 'Authorization:${API_KEY}' -H 'Content-Type: application/octet-stream' https://rubygems.org/api/v1/gems"""
        }
}
